---
description: "Swimming Pool permits in Irwindale, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Irwindale local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Irwindale."
version: "1.0"
...
# Irwindale — Swimming Pool Permit

Handles swimming pool permits in Irwindale end-to-end.
...
```
## Ready to Submit — Irwindale Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/irwindale/swimming-pool/`.
