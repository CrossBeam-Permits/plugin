---
description: "Pool Demolition permits in San Juan Bautista, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + San Juan Bautista local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions pool demolition in San Juan Bautista."
version: "1.0"
...
# City of San Juan Bautista — Pool Demolition Permit

Handles pool demolition permits in City of San Juan Bautista end-to-end.
...
```
## Ready to Submit — City of San Juan Bautista Pool Demolition Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/pool-demolition/`. City overlay source: `skills/cities/san-juan-bautista/pool-demolition/`.
