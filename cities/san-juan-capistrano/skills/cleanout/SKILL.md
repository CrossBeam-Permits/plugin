---
description: "Cleanout permits in San Juan Capistrano, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + San Juan Capistrano local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions cleanout in San Juan Capistrano."
version: "1.0"
...
# San Juan Capistrano — Cleanout Permit

Handles cleanout permits in San Juan Capistrano end-to-end.
...
```
## Ready to Submit — San Juan Capistrano Cleanout Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/cleanout/`. City overlay source: `skills/cities/san-juan-capistrano/cleanout/`.
