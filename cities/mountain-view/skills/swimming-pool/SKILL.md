---
description: "Swimming Pool permits in Mountain View, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Mountain View local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Mountain View."
version: "1.0"
...
# Mountain View — Swimming Pool Permit

Handles swimming pool permits in Mountain View end-to-end.
...
```
## Ready to Submit — Mountain View Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/mountain-view/swimming-pool/`.
