---
description: "Rewire permits in Rolling Hills Estates, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rolling Hills Estates local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Rolling Hills Estates."
version: "1.0"
...
# Rolling Hills Estates — Rewire Permit

Handles rewire permits in Rolling Hills Estates end-to-end.
...
```
## Ready to Submit — Rolling Hills Estates Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/rolling-hills-estates/rewire/`.
