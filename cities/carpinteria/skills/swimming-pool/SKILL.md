---
description: "Swimming Pool permits in Carpinteria, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Carpinteria local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Carpinteria."
version: "1.0"
...
# Carpinteria — Swimming Pool Permit

Handles swimming pool permits in Carpinteria end-to-end.
...
```
## Ready to Submit — Carpinteria Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/carpinteria/swimming-pool/`.
