---
description: "Ltia Demolition permits in Hughson, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Hughson local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions ltia demolition in Hughson."
version: "1.0"
...
# Hughson — Ltia Demolition Permit

Handles ltia demolition permits in Hughson end-to-end.
...
```
## Ready to Submit — Hughson Ltia Demolition Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/ltia-demolition/`. City overlay source: `skills/cities/hughson/ltia-demolition/`.
