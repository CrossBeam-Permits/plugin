---
description: "Rewire permits in Sutter Creek, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Sutter Creek local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Sutter Creek."
version: "1.0"
...
# Sutter Creek — Rewire Permit

Handles rewire permits in Sutter Creek end-to-end.
...
```
## Ready to Submit — Sutter Creek Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/sutter-creek/rewire/`.
