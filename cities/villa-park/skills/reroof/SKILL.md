---
description: "Reroof permits in Villa Park, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Villa Park local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Villa Park."
version: "1.0"
...
# Villa Park — Reroof Permit

Handles reroof permits in Villa Park end-to-end.
...
- **1** VHFHSZ — roof vents — **flag**
- **2** VHFHSZ — underlayment under air-space profiles — **flag**
- **3** OCFA Class B floor — **flag**
- **4** OTC eligibility — **flag**
- **5** Counter availability — critical — **flag**
- **6** Construction hours — **flag**
- **7** Code cycle (effective Jan 1, 2026) — **flag**
- **8** Plan submission format — **flag**
- **9** Local amendments (pending full scrape) — **flag**
...
```
## Ready to Submit — Villa Park Reroof Permit
...
| 1 | **VHFHSZ — roof vents** `[VPMC]` | ⚠️ **flag** | In VHFHSZ areas, all new or replaced roof vents must be ember-resistant (1/16" mesh, no openings >1/8") per Chapter 7A; when >50% of vents are removed/replaced during reroof, all vents must be upgraded to fire-rated/ember-resistant | CRC R337.6 / CBC 706A; Gov. Code 51182 |
| 2 | **VHFHSZ — underlayment under air-space profiles** `[VPMC]` | ⚠️ **flag** | Where roofing profile creates air space below material (e.g., tile), underlayment must comply with R337.5 / CBC 705A in VHFHSZ | CRC R337.5 / CBC 705A |
| 3 | **OCFA Class B floor** `[VPMC]` | ⚠️ **flag** | OCFA requires minimum Class B roofing for ALL construction types citywide (raises the state Type IIB/IIIB/VB Class C floor to Class B) | OCFA Standard Delta #4 |
| 4 | **OTC eligibility** `[VPMC]` | ⚠️ **flag** | Re-roof permits are eligible for OTC (over-the-counter) issuance; submit online via iWorQ portal or in person at counter | City process (iWorQ portal) |
| 5 | **Counter availability — critical** `[VPMC]` | ⚠️ **flag** | Counter open Mon / Wed / Fri 8:00–11:00 AM ONLY. Inspections Mon / Wed / Fri 11:00 AM–2:00 PM. 24-hour advance notice required for inspection scheduling. | City B&S Department |
| 6 | **Construction hours** `[VPMC]` | ⚠️ **flag** | Mon–Fri 7:00 AM–8:00 PM; Sat 8:00 AM–8:00 PM; no construction Sundays; no work on federal holidays | Villa Park B&S Department (confirmed) |
| 7 | **Code cycle (effective Jan 1, 2026)** `[VPMC]` | ⚠️ **flag** | All permits submitted on or after Jan 1, 2026 use 2025 California Building Standards Code (Title 24) | Villa Park B&S website (confirmed) |
| 8 | **Plan submission format** `[VPMC]` | ⚠️ **flag** | All plans submitted online as PDF via iWorQ portal, or on flash drive at counter; no hand-drawn/hand-written submittals (consistent with OC practice) | City policy (B&S website) |
| 9 | **Local amendments (pending full scrape)** `[VPMC]` | ⚠️ **flag** | VPMC Chapter IX (Art. 9-2) contains local CBC/CRC amendments. Full text not yet scraped due to Municode JS rendering. Verify at https://library.municode.com/ca/villa_park (nodeId: CHIXCACO) for any additional roofing, fire zone, or overlay amendments before finalizing review. | VPMC Ch. IX |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/villa-park/reroof/`.
