---
description: "Swimming Pool permits in San Luis Obispo, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + San Luis Obispo local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in San Luis Obispo."
version: "1.0"
...
# San Luis Obispo — Swimming Pool Permit

Handles swimming pool permits in San Luis Obispo end-to-end.
...
```
## Ready to Submit — San Luis Obispo Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/san-luis-obispo/swimming-pool/`.
