---
description: "Fireworks Temp Use permits in Antioch, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Antioch local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions fireworks temp use in Antioch."
version: "1.0"
...
# Antioch — Fireworks Temp Use Permit

Handles fireworks temp use permits in Antioch end-to-end.
...
```
## Ready to Submit — Antioch Fireworks Temp Use Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/fireworks-temp-use/`. City overlay source: `skills/cities/antioch/fireworks-temp-use/`.
