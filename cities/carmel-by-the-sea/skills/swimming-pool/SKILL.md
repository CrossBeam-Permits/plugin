---
description: "Swimming Pool permits in Carmel By The Sea, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Carmel By The Sea local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Carmel By The Sea."
version: "1.0"
...
# Carmel-by-the-Sea — Swimming Pool Permit

Handles swimming pool permits in Carmel-by-the-Sea end-to-end.
...
```
## Ready to Submit — Carmel-by-the-Sea Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/carmel-by-the-sea/swimming-pool/`.
