---
description: "Ltia Demolition permits in Point Arena, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Point Arena local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions ltia demolition in Point Arena."
version: "1.0"
...
# Point Arena — Ltia Demolition Permit

Handles ltia demolition permits in Point Arena end-to-end.
...
```
## Ready to Submit — Point Arena Ltia Demolition Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/ltia-demolition/`. City overlay source: `skills/cities/point-arena/ltia-demolition/`.
