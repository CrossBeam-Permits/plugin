---
description: "Swimming Pool permits in Yorba Linda, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Yorba Linda local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Yorba Linda."
version: "1.0"
...
# Yorba Linda — Swimming Pool Permit

Handles swimming pool permits in Yorba Linda end-to-end.
...
- **2** Pet doors prohibited — **flag**
- **3** 5 ft deck surround — **flag**
- **4** 18" depth trigger — **flag**
- **5** Pool barrier fence height adjustment — **flag**
- **6** Construction hours — **flag**
- **7** Electronic submittal — **flag**
- **8** Permit fees — **flag**
- **9** Special hazards screening — **flag**
...
```
## Ready to Submit — Yorba Linda Swimming Pool Permit
...
| 1 | **Pre-site inspection** `[YLMC]` | ⚠️ **flag** | City-specific requirement BEFORE plan review starts; homeowner AND contractor must both be present | City swimming pool handout |
| 2 | **Pet doors prohibited** `[YLMC]` | ⚠️ **flag** | No pet doors in any door with direct pool/spa access | City swimming pool handout |
| 3 | **5 ft deck surround** `[YLMC]` | ⚠️ **flag** | Minimum 5-foot-wide deck must surround each pool; must slope to drain away from pool | YLMC 15.32.020 |
| 4 | **18" depth trigger** `[YLMC]` | ⚠️ **flag** | Any constructed or prefabricated structure holding water >18 inches deep must comply with pool barrier requirements | Ord. 2022-1094 |
| 5 | **Pool barrier fence height adjustment** `[YLMC]` | ⚠️ **flag** | Community Development Director may approve height adjustment to meet 60" pool enclosure without formal variance | YLMC special provision |
| 6 | **Construction hours** `[YLMC]` | ⚠️ **flag** | Mon-Sat 7:00 AM - 8:00 PM; no construction Sundays or federal holidays | YLMC Ch. 8.32 |
| 7 | **Electronic submittal** `[YLMC]` | ⚠️ **flag** | Email to electronicprocessing@yorbalindaca.gov | City process |
| 8 | **Permit fees** `[YLMC]` | ⚠️ **flag** | Per City Council resolution (USPSHTC Table 104.5 deleted) | YLMC 15.32.020 |
| 9 | **Special hazards screening** `[YLMC]` | ⚠️ **flag** | Check for oil wells, faults, liquefaction — may affect pool placement and structural design | City procedure; see CITY.md |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/yorba-linda/swimming-pool/`.
