---
description: "Reroof permits in Yorba Linda, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Yorba Linda local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Yorba Linda."
version: "1.0"
...
# Yorba Linda — Reroof Permit

Handles reroof permits in Yorba Linda end-to-end.
...
- **1** Re-roof with solar — **flag**
- **2** OCFA 72-lb cap sheet — **flag**
- **3** OCFA vent replacement — **flag**
- **4** Underlayment in air-space profiles — **flag**
- **5** OTC eligibility — **flag**
...
```
## Ready to Submit — Yorba Linda Reroof Permit
...
| 1 | **Re-roof with solar** `[YLMC]` | ⚠️ **flag** | If solar panels must be removed for re-roof, a separate electrical permit is required for panel removal PRIOR to the roof permit | City re-roof handout |
| 2 | **OCFA 72-lb cap sheet** `[YLMC]` | ⚠️ **flag** | In WUI areas, 72-lb cap sheet required | OCFA fire zone requirement |
| 3 | **OCFA vent replacement** `[YLMC]` | ⚠️ **flag** | When >50% of roof vents are removed during re-roof, all vents must be replaced with fire-rated vents | OCFA / YLMC 15.05.050 |
| 4 | **Underlayment in air-space profiles** `[YLMC]` | ⚠️ **flag** | Where roofing material profile has air-space under covering, underlayment must comply with R337.5 / CBC 705A | YLMC 15.04.060, 15.05.050 |
| 5 | **OTC eligibility** `[YLMC]` | ⚠️ **flag** | Reroof is eligible for electronic OTC submittal; up to 10 working day review | City process |
...
| 8 | **No hand-written submittals** `[YLMC]` | ⚠️ **flag** | No hand-written information accepted on any plan submittal | City policy |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/yorba-linda/reroof/`.
