---
description: "Rewire permits in Rio Dell, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rio Dell local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Rio Dell."
version: "1.0"
...
# City of Rio Dell — Rewire Permit

Handles rewire permits in City of Rio Dell end-to-end.
...
```
## Ready to Submit — City of Rio Dell Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/rio-dell/rewire/`.
