---
description: "Fireworks Temp Use permits in Taft, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Taft local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions fireworks temp use in Taft."
version: "1.0"
...
# Taft — Fireworks Temp Use Permit

Handles fireworks temp use permits in Taft end-to-end.
...
```
## Ready to Submit — Taft Fireworks Temp Use Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/fireworks-temp-use/`. City overlay source: `skills/cities/taft/fireworks-temp-use/`.
