---
description: "Swimming Pool permits in Baldwin Park, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Baldwin Park local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Baldwin Park."
version: "1.0"
...
# Baldwin Park — Swimming Pool Permit

Handles swimming pool permits in Baldwin Park end-to-end.
...
```
## Ready to Submit — Baldwin Park Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/baldwin-park/swimming-pool/`.
