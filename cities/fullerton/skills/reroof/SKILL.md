---
description: "Reroof permits in Fullerton, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Fullerton local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Fullerton."
version: "1.0"
...
# Fullerton — Reroof Permit

Handles reroof permits in Fullerton end-to-end.
...
- **1** OTC permit path — **flag**
- **2** Portal — **flag**
- **3** HFHSZ parcel check — **flag**
- **4** Correct fire agency — **flag**
- **5** Cool roof — CZ 8 — **flag**
- **6** Contractor license — **flag**
- **7** Code cycle — **flag**
- **8** Permit validity — **flag**
...
```
## Ready to Submit — Fullerton Reroof Permit
...
| 1 | **OTC permit path** `[FMC]` | ⚠️ **flag** | Reroof is OTC (over-the-counter) via getaPERMIT.net or counter; no plan check required for standard residential/commercial reroof unless structural issues are present | Fullerton Building Division — permit type list |
| 2 | **Portal** `[FMC]` | ⚠️ **flag** | getaPERMIT.net — https://www.getapermit.net/fullerton; counter at 303 W. Commonwealth Avenue, Fullerton, CA 92832; Mon–Thu 7:30 AM – 5:30 PM, Fri 7:30 AM – 4:30 PM | City of Fullerton |
| 3 | **HFHSZ parcel check** `[FMC]` | ⚠️ **flag** | Northeastern foothills (Coyote Hills vicinity) are in HFHSZ. For any parcel in this area: (a) Class A roofing material required; (b) verify fire-rated vent replacement per CRC R337.6 if vents are disturbed; (c) coordinate with Fullerton Fire Department if fuel modification plan may apply | CAL FIRE FHSZ map; CRC R337.6; CFC Ch. 49 |
| 4 | **Correct fire agency** `[FMC]` | ⚠️ **flag** | Route all fire-related questions to **Fullerton Fire Department** — **NOT OCFA** | Fire Prevention Bureau: (714) 738-6500 |
| 5 | **Cool roof — CZ 8** `[FMC]` | ⚠️ **flag** | Climate Zone 8: steep-slope residential requires minimum aged solar reflectance 0.20 (or initial ≥ 0.25 + thermal emittance ≥ 0.75); low-slope commercial minimum aged solar reflectance 0.63. **Note:** These CZ 8 cool roof values are sourced from 2025 CEnC general requirements. Verify specific thresholds against 2025 CEnC Table 150.0-C for the applicable roof slope and occupancy type before finalizing compliance determination. | Title 24 Part 6, CZ 8 |
| 6 | **Contractor license** `[FMC]` | ⚠️ **flag** | C-39 (Roofing) license required; current CSLB license verification required at permit application | CSLB |
| 7 | **Code cycle** `[FMC]` | ⚠️ **flag** | All submittals on or after January 1, 2026 use **2025 California Building Standards Code** | 2025 CA BSC |
| 8 | **Permit validity** `[FMC]` | ⚠️ **flag** | 180 days to begin work after permit issuance; 1 year to complete — verify current policy with Building Division | FMC / local policy |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/fullerton/reroof/`.
