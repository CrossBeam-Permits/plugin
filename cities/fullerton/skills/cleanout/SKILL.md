---
description: "Cleanout permits in Fullerton, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Fullerton local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions cleanout in Fullerton."
version: "1.0"
...
# Fullerton — Cleanout Permit

Handles cleanout permits in Fullerton end-to-end.
...
- **1** Permit timing — before work commences — **flag**
...
```
## Ready to Submit — Fullerton Cleanout Permit
...
| 1 | **Permit timing — before work commences** `[FMC]` | ⚠️ **flag** | Permit must be obtained BEFORE work commences. Work performed without a permit requires an after-the-fact permit with investigation fees (typically double the standard permit fee). Previously concealed work may require re-exposure for inspection. Contact Building Division at (714) 738-6542 for after-the-fact permit procedures. | FMC; Fullerton Building Division |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/cleanout/`. City overlay source: `skills/cities/fullerton/cleanout/`.
