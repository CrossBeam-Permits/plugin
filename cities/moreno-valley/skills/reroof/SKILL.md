---
description: "Reroof permits in Moreno Valley, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Moreno Valley local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Moreno Valley."
version: "1.0"
...
# Moreno Valley — Reroof Permit

Handles reroof permits in Moreno Valley end-to-end.
...
- **1** Reroofing Certificate of Compliance — **flag**
- **2** FHSZ parcel verification — **flag**
- **3** Wind uplift — product rating — **flag**
- **4** Attic ventilation — **flag**
- **5** Permit portal — SimpliCITY — **flag**
...
```
## Ready to Submit — Moreno Valley Reroof Permit
...
| 1 | **Reroofing Certificate of Compliance** `[MVMC]` | ⚠️ **flag** | City-specific form required at submittal. Obtain from building@moval.org or through the SimpliCITY portal. Missing form is a submittal deficiency — permit cannot be issued without it | City of Moreno Valley Building and Safety Division requirement |
| 2 | **FHSZ parcel verification** `[MVMC]` | ⚠️ **flag** | Verify parcel FHSZ status using the city FHSZ map before applying roof fire classification requirements. Approximately 12,283 acres in Very High FHSZ — southern, eastern, and northern portions. Urban core parcels are generally NOT in FHSZ | FHSZ map: https://www.moreno-valley.ca.us/departments/fire/HighFireMap-Review.html |
| 3 | **Wind uplift — product rating** `[MVMC]` | ⚠️ **flag** | 110 mph Vult design wind (Exposure C). Roofing products must be rated and installed per manufacturer installation instructions for this wind zone. Document product wind resistance rating in submittal | ASCE 7-22; 2025 CRC Table R301.2 as applicable to Moreno Valley |
| 4 | **Attic ventilation** `[MVMC]` | ⚠️ **flag** | Hot inland summers regularly exceed 100°F. Verify ventilated attic requirements are met or unvented attic assembly documentation is provided per CRC R806 / CBC 1203.2. Replacing roof covering is an opportunity to confirm compliance | 2025 CRC R806; 2025 CBC 1203.2 |
| 5 | **Permit portal — SimpliCITY** `[MVMC]` | ⚠️ **flag** | All submittals through SimpliCITY (Accela) at https://aca-prod.accela.com/MOVAL/Default.aspx. In-person counter available at 14177 Frederick St, Moreno Valley CA 92552 | CITY.md — Permit Processing |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/moreno-valley/reroof/`.
