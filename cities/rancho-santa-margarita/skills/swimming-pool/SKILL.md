---
description: "Swimming Pool permits in Rancho Santa Margarita, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rancho Santa Margarita local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Rancho Santa Margarita."
version: "1.0"
...
# Rancho Santa Margarita — Swimming Pool Permit

Handles swimming pool permits in Rancho Santa Margarita end-to-end.
...
- **2** Bond beam inspection — **flag**
- **3** Equipment pad — **flag**
- **4** Electrical bonding — **flag**
...
```
## Ready to Submit — Rancho Santa Margarita Swimming Pool Permit
...
| 1 | **Barrier before water** `[RSMMC]` | ⚠️ **flag** | Pool barrier (fencing) must be in place and pass inspection prior to water fill / preplaster inspection. This is strictly enforced. | 2025 CBC Section 3109; RSM process |
| 2 | **Bond beam inspection** `[RSMMC]` | ⚠️ **flag** | Bond beam inspection required before gunite/shotcrete. | RSM inspection sequence |
| 3 | **Equipment pad** `[RSMMC]` | ⚠️ **flag** | Equipment pad location and grading inspected. Maintain required setbacks. | 2025 CBC; RSMMC |
| 4 | **Electrical bonding** `[RSMMC]` | ⚠️ **flag** | Electrical bonding of pool shell, equipment, and surrounding surfaces inspected per CEC Article 680. | 2025 CEC Article 680 |
| 5 | **Portal submittal** `[RSMMC]` | ⚠️ **flag** | Submit via CityTech Solutions (rsm.cts.city) or at counter. | City process |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/rancho-santa-margarita/swimming-pool/`.
