---
description: "Pool Demolition permits in Rancho Santa Margarita, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rancho Santa Margarita local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions pool demolition in Rancho Santa Margarita."
version: "1.0"
...
# Rancho Santa Margarita — Pool Demolition Permit

Handles pool demolition permits in Rancho Santa Margarita end-to-end.
...
```
## Ready to Submit — Rancho Santa Margarita Pool Demolition Permit
...
| 1 | **Standard procedure** `[RSMMC]` | ⚠️ **flag** | Remove decking and bond beam to 18 inches below grade, drill drain holes through pool floor, fill with 3/4-inch rock then compacted fill. | RSM standard procedure |
| 2 | **DigAlert 811** `[RSMMC]` | ⚠️ **flag** | DigAlert notification required prior to any demolition work. | Gov. Code Section 4216 |
| 3 | **Compaction report** `[RSMMC]` | ⚠️ **flag** | Compaction report required if the filled area will be built upon in the future. Soils engineer to certify compaction. | 2025 CBC; RSM process |
| 4 | **Multiple inspections** `[RSMMC]` | ⚠️ **flag** | Multiple inspections required throughout demolition process — pre-demo, drain holes, fill/compaction, final grade. | RSM inspection sequence |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/pool-demolition/`. City overlay source: `skills/cities/rancho-santa-margarita/pool-demolition/`.
