---
description: "Reroof permits in Rancho Santa Margarita, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rancho Santa Margarita local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Rancho Santa Margarita."
version: "1.0"
...
# Rancho Santa Margarita — Reroof Permit

Handles reroof permits in Rancho Santa Margarita end-to-end.
...
- **2** Class A in VHFHSZ — **flag**
- **3** OCFA fire coordination — **flag**
- **5** Smoke/CO affidavit — **flag**
...
```
## Ready to Submit — Rancho Santa Margarita Reroof Permit
...
| 1 | **VHFHSZ site verification** `[RSMMC]` | ⚠️ **flag** | Before applying fire classification rules, verify parcel-level FHSZ designation via OCFA GIS or CAL FIRE maps. RSM has mixed VHFHSZ coverage — do not assume all parcels are inside or outside the fire zone. | OCFA FHSZ maps; 2025 CBC Section 1505.1 |
| 2 | **Class A in VHFHSZ** `[RSMMC]` | ⚠️ **flag** | If parcel is in VHFHSZ, roof covering must be Class A fire-rated per CBC Section 1505. Class B/C not acceptable in VHFHSZ. | 2025 CBC Section 1505 |
| 3 | **OCFA fire coordination** `[RSMMC]` | ⚠️ **flag** | For reroof in VHFHSZ, OCFA may require fire-rated vent replacement (CBC 706A / CRC R337.6) when more than 50% of vents are removed. Confirm OCFA review requirements at permit intake. | OCFA plan check; 2025 CBC Chapter 7A |
| 5 | **Smoke/CO affidavit** `[RSMMC]` | ⚠️ **flag** | If interior access is not provided during reroof inspection, contractor must sign smoke/CO detector compliance affidavit at final. | RSM process; 2025 CRC R314/R315 |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/rancho-santa-margarita/reroof/`.
