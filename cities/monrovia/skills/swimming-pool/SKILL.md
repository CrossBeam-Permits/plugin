---
description: "Swimming Pool permits in Monrovia, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Monrovia local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Monrovia."
version: "1.0"
...
# Monrovia — Swimming Pool Permit

Handles swimming pool permits in Monrovia end-to-end.
...
```
## Ready to Submit — Monrovia Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/monrovia/swimming-pool/`.
