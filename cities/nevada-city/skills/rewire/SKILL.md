---
description: "Rewire permits in Nevada City, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Nevada City local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Nevada City."
version: "1.0"
...
# Nevada City — Rewire Permit

Handles rewire permits in Nevada City end-to-end.
...
```
## Ready to Submit — Nevada City Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/nevada-city/rewire/`.
