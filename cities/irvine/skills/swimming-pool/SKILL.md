---
description: "Swimming Pool permits in Irvine, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Irvine local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Irvine."
version: "1.0"
...
# Irvine — Swimming Pool Permit

Handles swimming pool permits in Irvine end-to-end.
...
- **1** Barrier before water — **flag**
- **2** HOA approval — **flag**
- **3** OCFA coordination — **flag**
- **4** IRWD coordination — **flag**
- **5** VHFHSZ check — **flag**
- **6** Underground wiring — **flag**
- **7** Equipment electrical bonding — **flag**
- **8** Bond beam inspection — **flag**
- **9** No automatic water softeners — **flag**
- **10** Portal submittal — **flag**
...
```
## Ready to Submit — Irvine Swimming Pool Permit
...
| 1 | **Barrier before water** `[IMC]` | ⚠️ **flag** | Pool barrier (fencing) must be in place and pass inspection prior to water fill / preplaster inspection. Strictly enforced. | 2025 CBC Section 3109; Irvine process |
| 2 | **HOA approval** `[IMC]` | ⚠️ **flag** | Most Irvine neighborhoods are in HOA communities. Verify HOA ARC approval obtained before proceeding — separate from city permit. | Master-planned community overlay |
| 3 | **OCFA coordination** `[IMC]` | ⚠️ **flag** | If pool house or accessory structure includes fire sprinkler system, OCFA plan check required (separate submittal and fee). | OCFA; Irvine CFC amendment |
| 4 | **IRWD coordination** `[IMC]` | ⚠️ **flag** | If work involves water service connection or impacts IRWD infrastructure, coordinate with Irvine Ranch Water District (separate agency). | IRWD |
| 5 | **VHFHSZ check** `[IMC]` | ⚠️ **flag** | Eastern foothill parcels (near Limestone Canyon, Bommer Canyon, Irvine Open Space Preserve) may be in VHFHSZ. If pool house or accessory structure is part of scope, verify FHSZ designation — CBC Chapter 7A fire-resistant construction may apply to the structure. | 2025 CBC Chapter 7A; CAL FIRE FHSZ maps |
| 6 | **Underground wiring** `[IMC]` | ⚠️ **flag** | All exterior electrical for pool equipment (pumps, heaters, lighting) must be underground — no overhead wiring on private property. | Irvine CEC local amendment |
| 7 | **Equipment electrical bonding** `[IMC]` | ⚠️ **flag** | Electrical bonding of pool shell, equipment, and surrounding surfaces per CEC Article 680. | 2025 CEC Article 680 |
| 8 | **Bond beam inspection** `[IMC]` | ⚠️ **flag** | Bond beam inspection required before gunite/shotcrete. | Irvine inspection sequence |
| 9 | **No automatic water softeners** `[IMC]` | ⚠️ **flag** | Automatic water softeners prohibited on all new work (Irvine local amendment). | IMC local amendment |
| 10 | **Portal submittal** `[IMC]` | ⚠️ **flag** | Submit via IrvineREADY! GoPost (https://gopost-irvine.eplansoftreview.com) — fully electronic. | City process |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/irvine/swimming-pool/`.
