...
# Irvine — Pool Demolition Permit

Handles pool demolition permits in Irvine end-to-end.
...
- **1** Standard procedure — **flag**
- **2** DigAlert 811 — **flag**
- **3** Compaction report — **flag**
- **4** HOA notification — **flag**
- **5** IRWD coordination — **flag**
- **6** Multiple inspections — **flag**
...
```
## Ready to Submit — Irvine Pool Demolition Permit
...
| 1 | **Standard procedure** `[IMC]` | ⚠️ **flag** | Remove decking and bond beam to 18 inches below grade, drill drain holes through pool floor, fill with 3/4-inch rock then compacted fill. | Irvine standard procedure |
| 2 | **DigAlert 811** `[IMC]` | ⚠️ **flag** | DigAlert notification required prior to any demolition work. | Gov. Code Section 4216 |
| 3 | **Compaction report** `[IMC]` | ⚠️ **flag** | Compaction report required if the filled area will be built upon in the future. Soils engineer to certify compaction. | 2025 CBC; Irvine process |
| 4 | **HOA notification** `[IMC]` | ⚠️ **flag** | Most Irvine neighborhoods are in HOA communities. Notify HOA of demolition — some HOAs require ARC review even for demolition. | Master-planned community overlay |
| 5 | **IRWD coordination** `[IMC]` | ⚠️ **flag** | If pool demolition affects water service or sewer lateral, coordinate with Irvine Ranch Water District. | IRWD |
| 6 | **Multiple inspections** `[IMC]` | ⚠️ **flag** | Multiple inspections required: pre-demo, drain holes, fill/compaction, final grade. | Irvine inspection sequence |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/pool-demolition/`. City overlay source: `skills/cities/irvine/pool-demolition/`.
