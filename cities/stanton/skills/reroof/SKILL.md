---
description: "Reroof permits in Stanton, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Stanton local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Stanton."
version: "1.0"
...
# Stanton — Reroof Permit

Handles reroof permits in Stanton end-to-end.
...
```
## Ready to Submit — Stanton Reroof Permit
...
| 1 | **City Reroofing Guide** `[SMC]` | ⚠️ **flag** | Stanton Building Division publishes a "Reroofing Guide" handout — verify submittal follows guide requirements | stantonca.gov applications page |
| 2 | **Portal submission** `[SMC]` | ⚠️ **flag** | Submit via stanton.cts.city | — |
| 3 | **Not VHFHSZ** `[SMC]` | ⚠️ **flag** | Stanton is flatland urban — not in a Very High Fire Hazard Severity Zone; standard roofing fire ratings apply (CBC Table 1505.1) | — |
| 5 | **4-day work week** `[SMC]` | ⚠️ **flag** | Inspections Mon–Thu only | Building Division |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/stanton/reroof/`.
