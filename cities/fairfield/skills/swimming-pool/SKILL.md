---
description: "Swimming Pool permits in Fairfield, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Fairfield local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Fairfield."
version: "1.0"
...
# Fairfield — Swimming Pool Permit

Handles swimming pool permits in Fairfield end-to-end.
...
```
## Ready to Submit — Fairfield Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/fairfield/swimming-pool/`.
