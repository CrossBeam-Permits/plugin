---
description: "Cleanout permits in Torrance, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Torrance local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions cleanout in Torrance."
version: "1.0"
...
# Torrance — Cleanout Permit

Handles cleanout permits in Torrance end-to-end.
...
```
## Ready to Submit — Torrance Cleanout Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/cleanout/`. City overlay source: `skills/cities/torrance/cleanout/`.
