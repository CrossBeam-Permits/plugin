---
description: "Rewire permits in Lomita, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Lomita local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Lomita."
version: "1.0"
...
# Lomita — Rewire Permit

Handles rewire permits in Lomita end-to-end.
...
```
## Ready to Submit — Lomita Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/lomita/rewire/`.
