---
description: "Swimming Pool permits in Fountain Valley, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Fountain Valley local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Fountain Valley."
version: "1.0"
...
# Fountain Valley — Swimming Pool Permit

Handles swimming pool permits in Fountain Valley end-to-end.
...
- **1** 2025 code edition on all sheets — **flag**
- **2** Pool/spa-to-structure setback — **flag**
- **3** CTS portal submission — **flag**
- **4** Separate fire plan submittal — **flag**
- **5** Correct fire agency — **flag**
- **6** PE-stamped structural plans — **flag**
- **7** No coastal / FHSZ triggers — **flag**
- **8** SDC D seismic design — **flag**
- **9** Title 24 Part 6 — pool heater — **flag**
- **10** Counter hours — no Friday walk-in — **flag**
...
```
## Ready to Submit — Fountain Valley Swimming Pool Permit
...
| 1 | **2025 code edition on all sheets** `[FVMC]` | ⚠️ **flag** | Every plan sheet must cite the 2025 California Building Standards Code — confirm CBC, CRC, CEC, CPC, CMC citations on title sheet | FVMC Title 18 |
| 2 | **Pool/spa-to-structure setback** `[FVMC]` | ⚠️ **flag** | Verify minimum 5 ft from house structure (foundation face or wall) to pool/spa water line shown on site plan | Local standard (FVMC) |
| 3 | **CTS portal submission** `[FVMC]` | ⚠️ **flag** | Pool permit applied for, tracked, and paid via CTS at https://fountainvalley.cts.city | Fountain Valley permit path |
| 4 | **Separate fire plan submittal** `[FVMC]` | ⚠️ **flag** | Confirm applicant understands fire plan review requires a separate paper submission to FVFD — not through CTS | CITY.md |
| 5 | **Correct fire agency** `[FVMC]` | ⚠️ **flag** | All fire-related plan review questions and submittals route to Fountain Valley Fire Department — NOT OCFA | CITY.md |
| 6 | **PE-stamped structural plans** `[FVMC]` | ⚠️ **flag** | Pool shell structural design must bear a licensed California structural or civil engineer stamp — no prescriptive path in SDC D | 2025 CBC § 1807.1.6 |
| 7 | **No coastal / FHSZ triggers** `[FVMC]` | ⚠️ **flag** | Confirm no CDP required (not coastal) and no fuel modification plan required (no FHSZ) — standard inland residential review applies | CITY.md |
| 8 | **SDC D seismic design** `[FVMC]` | ⚠️ **flag** | Pool shell and any retaining walls or associated structures must be designed for SDC D seismic loads | 2025 CBC (ASCE 7-16) |
| 9 | **Title 24 Part 6 — pool heater** `[FVMC]` | ⚠️ **flag** | Pool/spa heater (gas or electric) must comply with 2025 California Energy Code (Title 24, Part 6) §110.4 — heater cover required on outdoor units | 2025 CEnC § 110.4 |
| 10 | **Counter hours — no Friday walk-in** `[FVMC]` | ⚠️ **flag** | Plan counter is Monday–Thursday 7:00 AM – 6:00 PM only; closed Fridays — advise applicants accordingly | CITY.md |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/fountain-valley/swimming-pool/`.
