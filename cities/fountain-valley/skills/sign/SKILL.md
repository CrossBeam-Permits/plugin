...
# Fountain Valley — Sign Permit

Handles sign permits in Fountain Valley end-to-end.
...
- **1** Written Planning Department approval first — **flag**
- **2** Correct code cycle — **flag**
- **3** SDC D seismic design — **flag**
- **4** Wind design — 95 mph Vult — **flag**
- **5** Fire review — FVFD only — **flag**
- **6** CTS portal — **flag**
- **7** Counter hours — no Fridays — **flag**
- **8** Temporary sign permit — **flag**
- **9** FVMC Title 21 zone specifics — **flag**
...
```
## Ready to Submit — Fountain Valley Sign Permit
...
| 1 | **Written Planning Department approval first** `[FVMC]` | ⚠️ **flag** | All signs require **written** Planning Department zoning approval (approval letter or stamped plan set) for FVMC Title 21 compliance before Building Division sign permit is issued — verbal or email clearance is not sufficient | FVMC Title 21 |
| 2 | **Correct code cycle** `[FVMC]` | ⚠️ **flag** | Plans must cite 2025 California Building Standards Code | FVMC Title 18 |
| 3 | **SDC D seismic design** `[FVMC]` | ⚠️ **flag** | All structural sign designs must be designed for Seismic Design Category D; verify attachment, anchorage, and foundation details accordingly | CBC 1613 / ASCE 7 |
| 4 | **Wind design — 95 mph Vult** `[FVMC]` | ⚠️ **flag** | Default wind speed ~95 mph Vult (RC II); verify for specific parcel using ASCE 7 Hazard Tool; Wind Exposure C | CBC 1609 / ASCE 7 |
| 5 | **Fire review — FVFD only** `[FVMC]` | ⚠️ **flag** | Route any fire/electrical sign review to Fountain Valley Fire Department via separate paper plan submission — NOT OCFA | City policy |
| 6 | **CTS portal** `[FVMC]` | ⚠️ **flag** | Building sign permit submitted at https://fountainvalley.cts.city — not through a city counter form | City policy |
| 7 | **Counter hours — no Fridays** `[FVMC]` | ⚠️ **flag** | Counter is Monday–Thursday 7:00 AM – 6:00 PM; all permit counter visits and drop-offs must be Mon–Thu | City policy |
| 8 | **Temporary sign permit** `[FVMC]` | ⚠️ **flag** | Temporary banners, promotional signs, and grand opening signs require separate approval from Planning Department — not covered by permanent sign permit | FVMC Title 21 |
| 9 | **FVMC Title 21 zone specifics** `[FVMC]` | ⚠️ **flag** | Full sign code text not retrieved — always contact Planning for allowable sign area, height, setbacks, illumination standards, and sign count limits for specific zone | FVMC Title 21 (eCode360 WAF-blocked) |
| 10 | **No coastal jurisdiction** `[FVMC]` | ⚠️ **flag** | Fountain Valley is not in the Coastal Zone — no Coastal Commission permit required for any sign project | — |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/sign/`. City overlay source: `skills/cities/fountain-valley/sign/`.
