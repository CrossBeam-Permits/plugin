---
description: "Reroof permits in Fountain Valley, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Fountain Valley local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Fountain Valley."
version: "1.0"
...
# Fountain Valley — Reroof Permit

Handles reroof permits in Fountain Valley end-to-end.
...
- **1** Permit portal — **flag**
- **2** Counter hours — **flag**
- **3** Code cycle — 2025 CRC — **flag**
- **4** Fire agency — **flag**
- **5** FHSZ status — **flag**
- **7** Seismic Design Category D — **flag**
- **8** Local amendments — WAF-blocked — **flag**
- **9** Contractor license — **flag**
- **10** No coastal jurisdiction — **flag**
...
```
## Ready to Submit — Fountain Valley Reroof Permit
...
| 1 | **Permit portal** `[FVMC]` | ⚠️ **flag** | Submit via CTS — https://fountainvalley.cts.city. Reroof is an expedited permit type; OTC approval available online | CTS Permit Center |
| 2 | **Counter hours** `[FVMC]` | ⚠️ **flag** | Monday–Thursday, 7:00 AM – 6:00 PM. **Closed Friday**, weekends, and holidays. Schedule inspections accordingly | City of Fountain Valley |
| 3 | **Code cycle — 2025 CRC** `[FVMC]` | ⚠️ **flag** | All code citations must reference the **2025** California Residential Code. Fountain Valley has adopted the 2025 California Building Standards Code. Reject any document citing 2025 CRC, 2025 CBC, or 2025 CEnC | FVMC Title 18 |
| 4 | **Fire agency** `[FVMC]` | ⚠️ **flag** | Route fire questions to **Fountain Valley Fire Department** — **NOT OCFA**. FVFD operates three stations (Euclid St, Brookhurst St, Warner Ave). Fire plan review, if required, requires **separate paper submission to FVFD** — it does not go through the CTS portal | City fire department |
| 5 | **FHSZ status** `[FVMC]` | ⚠️ **flag** | Fountain Valley is **not** in a state-designated FHSZ or VHFHSZ. Class A roofing is not required on that basis. If the applicant's address is near the city boundary, confirm with FVFD that no site-specific fire hazard determination applies | CAL FIRE FHSZ map; FVFD |
| 7 | **Seismic Design Category D** `[FVMC]` | ⚠️ **flag** | SDC D governs — standard SDC D design requirements apply to any structural work. Reroof-only with no structural changes is unaffected, but note if structural sheathing repairs are in scope | FVMC / ASCE 7-16 |
| 8 | **Local amendments — WAF-blocked** `[FVMC]` | ⚠️ **flag** | FVMC eCode360 (https://ecode360.com/FO4927) is WAF-blocked. Known amendments are limited; call Building Division at (714) 593-4429 to confirm any roofing-specific local amendments before issuing a definitive pass/fail on fire classification | FVMC Title 18 |
| 9 | **Contractor license** `[FVMC]` | ⚠️ **flag** | C-39 (Roofing) preferred; B (General Building) acceptable. Current CSLB license required. Workers' compensation insurance required | B&P Code 7028 |
| 10 | **No coastal jurisdiction** `[FVMC]` | ⚠️ **flag** | Fountain Valley is an inland city — no California Coastal Commission permit required | Coastal Act |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/fountain-valley/reroof/`.
