---
description: "Hvac permits in Hidden Hills, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Hidden Hills local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions hvac in Hidden Hills."
version: "1.0"
...
# Hidden Hills — Hvac Permit

Handles hvac permits in Hidden Hills end-to-end.
...
```
## Ready to Submit — Hidden Hills Hvac Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/hvac/`. City overlay source: `skills/cities/hidden-hills/hvac/`.
