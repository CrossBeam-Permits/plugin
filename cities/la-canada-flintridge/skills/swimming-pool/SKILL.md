---
description: "Swimming Pool permits in La Canada Flintridge, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + La Canada Flintridge local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in La Canada Flintridge."
version: "1.0"
...
# La Canada Flintridge — Swimming Pool Permit

Handles swimming pool permits in La Canada Flintridge end-to-end.
...
```
## Ready to Submit — La Canada Flintridge Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/la-canada-flintridge/swimming-pool/`.
