---
description: "Cleanout permits in Rancho Mirage, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rancho Mirage local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions cleanout in Rancho Mirage."
version: "1.0"
...
# Rancho Mirage — Cleanout Permit

Handles cleanout permits in Rancho Mirage end-to-end.
...
```
## Ready to Submit — Rancho Mirage Cleanout Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/cleanout/`. City overlay source: `skills/cities/rancho-mirage/cleanout/`.
