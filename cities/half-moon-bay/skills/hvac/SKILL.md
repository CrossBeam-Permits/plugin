---
description: "Hvac permits in Half Moon Bay, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Half Moon Bay local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions hvac in Half Moon Bay."
version: "1.0"
...
# Half Moon Bay — Hvac Permit

Handles hvac permits in Half Moon Bay end-to-end.
...
```
## Ready to Submit — Half Moon Bay Hvac Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/hvac/`. City overlay source: `skills/cities/half-moon-bay/hvac/`.
