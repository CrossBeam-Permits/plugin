---
description: "Reroof permits in Rancho Palos Verdes, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rancho Palos Verdes local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Rancho Palos Verdes."
version: "1.0"
...
# Rancho Palos Verdes — Reroof Permit

Handles reroof permits in Rancho Palos Verdes end-to-end.
...
```
## Ready to Submit — Rancho Palos Verdes Reroof Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/rancho-palos-verdes/reroof/`.
