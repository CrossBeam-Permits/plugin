---
description: "Rewire permits in Westmorland, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Westmorland local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Westmorland."
version: "1.0"
...
# Westmorland — Rewire Permit

Handles rewire permits in Westmorland end-to-end.
...
```
## Ready to Submit — Westmorland Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/westmorland/rewire/`.
