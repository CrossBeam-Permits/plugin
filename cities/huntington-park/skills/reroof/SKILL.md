---
description: "Reroof permits in Huntington Park, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Huntington Park local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Huntington Park."
version: "1.0"
...
# Huntington Park — Reroof Permit

Handles reroof permits in Huntington Park end-to-end.
...
```
## Ready to Submit — Huntington Park Reroof Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/huntington-park/reroof/`.
