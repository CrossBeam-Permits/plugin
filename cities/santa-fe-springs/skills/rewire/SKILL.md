---
description: "Rewire permits in Santa Fe Springs, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Santa Fe Springs local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Santa Fe Springs."
version: "1.0"
...
# Santa Fe Springs — Rewire Permit

Handles rewire permits in Santa Fe Springs end-to-end.
...
```
## Ready to Submit — Santa Fe Springs Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/santa-fe-springs/rewire/`.
