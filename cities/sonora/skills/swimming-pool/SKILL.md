---
description: "Swimming Pool permits in Sonora, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Sonora local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Sonora."
version: "1.0"
...
# City of Sonora — Swimming Pool Permit

Handles swimming pool permits in City of Sonora end-to-end.
...
```
## Ready to Submit — City of Sonora Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/sonora/swimming-pool/`.
