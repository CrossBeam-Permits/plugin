---
description: "Rewire permits in Garden Grove, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Garden Grove local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Garden Grove."
version: "1.0"
...
# Garden Grove — Rewire Permit

Handles rewire permits in Garden Grove end-to-end.
...
```
## Ready to Submit — Garden Grove Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/garden-grove/rewire/`.
