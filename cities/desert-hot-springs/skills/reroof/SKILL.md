---
description: "Reroof permits in Desert Hot Springs, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Desert Hot Springs local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Desert Hot Springs."
version: "1.0"
...
# Desert Hot Springs — Reroof Permit

Handles reroof permits in Desert Hot Springs end-to-end.
...
```
## Ready to Submit — Desert Hot Springs Reroof Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/desert-hot-springs/reroof/`.
