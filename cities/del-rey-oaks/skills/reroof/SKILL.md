---
description: "Reroof permits in Del Rey Oaks, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Del Rey Oaks local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Del Rey Oaks."
version: "1.0"
...
# Del Rey Oaks — Reroof Permit

Handles reroof permits in Del Rey Oaks end-to-end.
...
```
## Ready to Submit — Del Rey Oaks Reroof Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/del-rey-oaks/reroof/`.
