---
description: "Swimming Pool permits in Palos Verdes Estates, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Palos Verdes Estates local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Palos Verdes Estates."
version: "1.0"
...
# Palos Verdes Estates — Swimming Pool Permit

Handles swimming pool permits in Palos Verdes Estates end-to-end.
...
```
## Ready to Submit — Palos Verdes Estates Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/palos-verdes-estates/swimming-pool/`.
