---
description: "Swimming Pool permits in Brea, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Brea local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Brea."
version: "1.0"
...
# Brea — Swimming Pool Permit

Handles swimming pool permits in Brea end-to-end.
...
- **1** Standard Plan 02 referenced — **flag**
- **2** Standard Plan 03 referenced (demolition only) — **flag**
- **3** Standard Plan 04 referenced — **flag**
- **4** PE-stamped structural plans — **flag**
- **5** Geotechnical report check — **flag**
- **6** Correct fire agency — **flag**
- **7** HFHSZ parcel check — **flag**
- **8** Accela portal — **flag**
- **9** 2025 code edition — **flag**
...
```
## Ready to Submit — Brea Swimming Pool Permit
...
| 1 | **Standard Plan 02 referenced** `[BMC]` | ⚠️ **flag** | Pool safety features standard plan cited on permit application | Brea Standard Plan 02 (DocCenter/641) |
| 2 | **Standard Plan 03 referenced (demolition only)** `[BMC]` | ⚠️ **flag** | Required for all pool demolition and backfill projects | Brea Standard Plan 03 (DocCenter/642) |
| 3 | **Standard Plan 04 referenced** `[BMC]` | ⚠️ **flag** | Pool enclosure standard plan cited on permit application | Brea Standard Plan 04 (DocCenter/643) |
| 4 | **PE-stamped structural plans** `[BMC]` | ⚠️ **flag** | Pool shell structural design must bear a licensed California structural or civil engineer stamp — no prescriptive path in SDC D | BMC § 15.08.020 / CBC § 1807.1.6 |
| 5 | **Geotechnical report check** `[BMC]` | ⚠️ **flag** | Determine if parcel has expansive soils or slope conditions — if so, geotechnical investigation required before structural design | CBC 2025 § 1803 |
| 6 | **Correct fire agency** `[BMC]` | ⚠️ **flag** | All fire-related plan review questions route to Brea Fire Department — NOT OCFA | Brea CITY.md |
| 7 | **HFHSZ parcel check** `[BMC]` | ⚠️ **flag** | Verify parcel is not in HFHSZ — if in HFHSZ, confirm fuel modification requirements with Brea Fire | 2025 CFC Ch. 49 |
| 8 | **Accela portal** `[BMC]` | ⚠️ **flag** | Permit applied for, tracked, and paid via Accela — https://aca-prod.accela.com/BREA/CommunityView | Brea permit path |
| 9 | **2025 code edition** `[BMC]` | ⚠️ **flag** | Plans on/after 1/1/2026 must cite 2025 California Building Standards Code | Brea CITY.md |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/brea/swimming-pool/`.
