---
description: "Reroof permits in Brea, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Brea local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Brea."
version: "1.0"
...
Handles reroof permits in Brea end-to-end.
...
- **2** Portal — **flag**
- **3** 33% threshold — prior work inquiry — **flag**
- **4** Solar system on existing roof — **flag**
- **5** HFHSZ site check — **flag**
...
```
## Ready to Submit — Brea Reroof Permit
...
| 1 | **OTC permit path** `[BMC]` | ⚠️ **flag** | Reroof is OTC (over-the-counter) at Accela; no plan check required for standard residential/commercial reroof unless structural issues are present | City permit type list |
| 2 | **Portal** `[BMC]` | ⚠️ **flag** | Accela — https://aca-prod.accela.com/BREA/CommunityView; counter at 1 Civic Center Circle, Mon–Thu 8 AM – 5 PM, alternate Fridays | City of Brea |
| 3 | **33% threshold — prior work inquiry** `[BMC]` | ⚠️ **flag** | Ask applicant: has any portion of this roof been replaced in the past 2 years? If yes, add that percentage to current scope; if combined total exceeds 33%, Class A is required | BMC § 15.08.020 |
| 4 | **Solar system on existing roof** `[BMC]` | ⚠️ **flag** | If a solar system exists on the roof, Standard Plan 15 (DocCenter/18906) governs the re-roof procedure; solar panel removal/reinstallation requires a separate solar permit | Standard Plan 15 |
| 5 | **HFHSZ site check** `[BMC]` | ⚠️ **flag** | Portions of Brea are in HFHSZ (city-designated, not VHFHSZ). For sites in HFHSZ: verify fire-rated vent replacement per CRC R337.6 if vents are disturbed | City FHSZ map; CRC R337.6 |
| 6 | **Correct fire agency** `[BMC]` | ⚠️ **flag** | Route fire questions to Brea Fire Department — **NOT OCFA** | City fire department |
| 8 | **Permit validity** `[BMC]` | ⚠️ **flag** | 180 days to begin work after permit issuance; 1 year to complete | BMC / local amendment |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/brea/reroof/`.
