---
description: "Rewire permits in Laguna Woods, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Laguna Woods local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Laguna Woods."
version: "1.0"
...
# Laguna Woods — Rewire Permit

Handles rewire permits in Laguna Woods end-to-end.
...
```
## Ready to Submit — Laguna Woods Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/laguna-woods/rewire/`.
