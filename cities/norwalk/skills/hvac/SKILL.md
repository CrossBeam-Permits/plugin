---
description: "Hvac permits in Norwalk, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Norwalk local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions hvac in Norwalk."
version: "1.0"
...
Handles hvac permits in Norwalk end-to-end.
...
```
## Ready to Submit — Norwalk Hvac Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/hvac/`. City overlay source: `skills/cities/norwalk/hvac/`.
