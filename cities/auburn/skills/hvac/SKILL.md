---
description: "Hvac permits in Auburn, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Auburn local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions hvac in Auburn."
version: "1.0"
...
Handles hvac permits in Auburn end-to-end.
...
```
## Ready to Submit — Auburn Hvac Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/hvac/`. City overlay source: `skills/cities/auburn/hvac/`.
