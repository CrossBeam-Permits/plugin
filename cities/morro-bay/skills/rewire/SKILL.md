---
description: "Rewire permits in Morro Bay, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Morro Bay local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Morro Bay."
version: "1.0"
...
# Morro Bay — Rewire Permit

Handles rewire permits in Morro Bay end-to-end.
...
```
## Ready to Submit — Morro Bay Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/morro-bay/rewire/`.
