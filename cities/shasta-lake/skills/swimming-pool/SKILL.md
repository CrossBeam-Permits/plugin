---
description: "Swimming Pool permits in Shasta Lake, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Shasta Lake local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Shasta Lake."
version: "1.0"
...
# City of Shasta Lake — Swimming Pool Permit

Handles swimming pool permits in City of Shasta Lake end-to-end.
...
```
## Ready to Submit — City of Shasta Lake Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/shasta-lake/swimming-pool/`.
