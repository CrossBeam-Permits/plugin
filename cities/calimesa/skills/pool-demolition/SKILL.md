...
# Calimesa — Pool Demolition Permit

Handles pool demolition permits in Calimesa end-to-end.
...
- **1** Swimming Pool Abandonment form — **flag**
- **2** Compaction testing — licensed geotech — **flag**
- **3** Backfill material — **flag**
- **4** SDC D — structural context — **flag**
- **5** Grading and drainage — **flag**
- **6** Utility caps — gas line — **flag**
- **7** Electrical removal — **flag**
- **8** Counter hours — narrow window — **flag**
- **9** VHFHSZ — no additional requirement for demolition — **flag**
...
```
## Ready to Submit — Calimesa Pool Demolition Permit
...
| 1 | **Swimming Pool Abandonment form** `[CMC]` | ⚠️ **flag** | Calimesa publishes a dedicated Swimming Pool Abandonment form. Download from calimesa.gov/166/Building-Safety (listed under both Demolition and Pools & Spas categories) and submit with the permit application. Failure to include this form may delay acceptance. | calimesa.gov/166/Building-Safety |
| 2 | **Compaction testing — licensed geotech** `[CMC]` | ⚠️ **flag** | Calimesa enforces compaction testing for pool backfill. A licensed geotechnical engineer or engineering geologist must observe compaction and provide a written report confirming 90–95% compaction per ASTM D1557. Report must be submitted to the city before the final inspection will be approved. | CBC 1705.6, J107.5; City practice |
| 3 | **Backfill material** `[CMC]` | ⚠️ **flag** | Standard process: pool drained, pool shell perforated with drainage holes (36" diameter per state skill), backfilled with gravel/sand/clean fill as specified. No organic material, large rocks, or construction debris. Top 18" must be suitable planting medium. Confirm backfill spec with Virginia Figueroa if using alternative material. | CBC J107.4; City practice |
| 4 | **SDC D — structural context** `[CMC]` | ⚠️ **flag** | While the pool is being removed, Calimesa's SDC D designation means the geotechnical report for adjacent structures (house, walls, fences) should confirm no differential settlement risk from the backfill operation. Flag if pool is within 5 ft of any structure foundation. | CMC Ch. 15.05; CBC Ch. 18 |
| 5 | **Grading and drainage** `[CMC]` | ⚠️ **flag** | Finished grade must drain at minimum 2% away from all existing structures. Because Calimesa has hillside areas, confirm drainage does not redirect toward a neighboring property or toward the street in a way that creates a new drainage problem. | CBC 1804.4 |
| 6 | **Utility caps — gas line** `[CMC]` | ⚠️ **flag** | Gas line to pool heater must be capped gastight at both the gas meter and the equipment location using a screw-joint fitting. This is inspected before backfill begins. | 2025 CPC 1206.3, 1210.8.1 |
| 7 | **Electrical removal** `[CMC]` | ⚠️ **flag** | All pool electrical sub-panel and associated conductors must be removed back to the service supply connection. Pool bonding grid must be disconnected. A separate electrical permit may be required if any pool circuits are being repurposed. | 2025 CEC 680.13 |
| 8 | **Counter hours — narrow window** `[CMC]` | ⚠️ **flag** | Counter is Mon–Thu 8:00 AM–2:30 PM, closed 12:00–12:30 PM. **Closed Friday.** Pool abandonment permits typically require plan check; budget accordingly. | CITY.md |
| 9 | **VHFHSZ — no additional requirement for demolition** `[CMC]` | ⚠️ **flag** | Being in a VHFHSZ does not create additional requirements for a pool demolition. However, any new planting or landscaping installed over the backfilled area may need to comply with defensible space requirements (CMC Ch. 15.10; CFC Ch. 49). | CITY.md |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/pool-demolition/`. City overlay source: `skills/cities/calimesa/pool-demolition/`.
