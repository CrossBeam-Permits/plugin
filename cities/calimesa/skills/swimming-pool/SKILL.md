---
description: "Swimming Pool permits in Calimesa, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Calimesa local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Calimesa."
version: "1.0"
...
# Calimesa — Swimming Pool Permit

Handles swimming pool permits in Calimesa end-to-end.
...
- **1** Swimming Pool and Spa Safety Barriers handout — **flag**
- **2** SDC D — structural calcs — **flag**
- **3** VHFHSZ parcels — **flag**
- **4** Slope setback — hillside areas — **flag**
- **5** Termite zone — Very Heavy — **flag**
- **6** Pool abandonment — **flag**
- **7** Counter hours — narrow window — **flag**
- **8** Fee schedule — **flag**
- **9** Calimesa Creek Overlay (CMC Ch. 18.40) — **flag**
...
```
## Ready to Submit — Calimesa Swimming Pool Permit
...
| 1 | **Swimming Pool and Spa Safety Barriers handout** `[CMC]` | ⚠️ **flag** | Calimesa publishes a "Swimming Pool and Spa Safety Barriers" handout. Obtain from city website or counter before submittal — it may contain locally specific barrier form or submittal requirements. | calimesa.gov/166/Building-Safety |
| 2 | **SDC D — structural calcs** `[CMC]` | ⚠️ **flag** | Pool shell design must account for Seismic Design Category D. For non-standard pool shapes, hillside sites, or pools near slopes, a PE-stamped structural analysis is required. Even for standard rectangular in-ground pools, the plan checker may request structural calcs confirming soil bearing and seismic loading assumptions. | CMC Ch. 15.05; CBC Ch. 16, 18 |
| 3 | **VHFHSZ parcels** `[CMC]` | ⚠️ **flag** | Portions of Calimesa are in Very High Fire Hazard Severity Zone (LRA). Pools on VHFHSZ parcels do not typically trigger additional pool-specific fire requirements, but any associated structures (pool house, covered patio, outdoor kitchen) must comply with CBC Chapter 7A ignition-resistant material requirements. Contact city for parcel-specific VHFHSZ status. | CBC Ch. 7A; CITY.md |
| 4 | **Slope setback — hillside areas** `[CMC]` | ⚠️ **flag** | Calimesa has hillside terrain. For pools near descending or ascending slopes, apply CBC 1808.7.3 slope setback rules. Pool wall within 7 ft horizontal of a slope top must support water without soil support. Geotechnical memo may be required if pool is within 15 ft of slope top or 8 ft of slope toe. | CBC 1808.7.3; 2025 CBC Ch. 18 |
| 5 | **Termite zone — Very Heavy** `[CMC]` | ⚠️ **flag** | Any wood framing associated with pool decking, fencing, or adjacent structures must comply with CBC requirements for Very Heavy termite damage probability zone (preservative treatment or naturally durable wood for ground-contact members). | CBC Ch. 23; CITY.md |
| 6 | **Pool abandonment** `[CMC]` | ⚠️ **flag** | When a pool is no longer wanted, a separate pool abandonment (demolition) permit is required. See the `pool-demolition` skill for the Calimesa-specific process. Calimesa publishes a Swimming Pool Abandonment form in both the Demolition and Pools & Spas categories on the city website. | calimesa.gov/166/Building-Safety |
| 7 | **Counter hours — narrow window** `[CMC]` | ⚠️ **flag** | Counter is Mon–Thu 8:00 AM–2:30 PM, closed 12:00–12:30 PM. **Closed Friday.** Pool permits typically require plan check; OTC same-day issuance is uncommon. Budget 1–2 weeks for plan check. | CITY.md |
| 8 | **Fee schedule** `[CMC]` | ⚠️ **flag** | Master Fee Schedule (effective Sept 1, 2025) at calimesa.gov/260. Pool permit fee amounts not confirmed — verify with Virginia Figueroa (ext. 226) at time of application. | City Master Fee Schedule |
| 9 | **Calimesa Creek Overlay (CMC Ch. 18.40)** `[CMC]` | ⚠️ **flag** | Calimesa Creek Overlay applies to parcels along Calimesa Creek. Pool permits on parcels within or adjacent to the creek overlay require environmental review. Verify parcel creek overlay status with city (buildingtech@calimesa.gov) before submittal. Pool drainage must not discharge into the creek — grading/drainage plan required for parcels near overlay boundary. | CMC Ch. 18.40; CITY.md |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/calimesa/swimming-pool/`.
