---
description: "Hvac permits in South El Monte, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + South El Monte local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions hvac in South El Monte."
version: "1.0"
...
# South El Monte — Hvac Permit

Handles hvac permits in South El Monte end-to-end.
...
```
## Ready to Submit — South El Monte Hvac Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/hvac/`. City overlay source: `skills/cities/south-el-monte/hvac/`.
