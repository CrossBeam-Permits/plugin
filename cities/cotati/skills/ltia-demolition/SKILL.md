---
description: "Ltia Demolition permits in Cotati, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Cotati local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions ltia demolition in Cotati."
version: "1.0"
...
# Cotati — Ltia Demolition Permit

Handles ltia demolition permits in Cotati end-to-end.
...
```
## Ready to Submit — Cotati Ltia Demolition Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/ltia-demolition/`. City overlay source: `skills/cities/cotati/ltia-demolition/`.
