---
description: "Swimming Pool permits in Imperial Beach, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Imperial Beach local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Imperial Beach."
version: "1.0"
...
# Imperial Beach — Swimming Pool Permit

Handles swimming pool permits in Imperial Beach end-to-end.
...
- **3** Barrier fencing handout reviewed — **flag**
- **4** Geotechnical review — high water table — **flag**
- **4a** FEMA flood zone check — **flag**
- **5** CSS portal submittal — **flag**
- **6** Building permit + electrical permit — **flag**
- **7** 2025 code edition referenced on plans — **flag**
...
```
## Ready to Submit — Imperial Beach Swimming Pool Permit
...
| 1 | **CDP obtained — Planning Division** `[IBMC]` | ⚠️ **flag** | City-issued CDP required for all new in-ground pool construction; submit to Planning Division concurrently with building permit application via CSS portal | LCP LCPA-23-0001 |
...
| 3 | **Barrier fencing handout reviewed** `[IBMC]` | ⚠️ **flag** | Obtain and apply city's Swimming Pool Barrier Fencing Handout from forms page; handout provides local guidance supplementing CBC standard | Imperial Beach Building Division |
| 4 | **Geotechnical review — high water table** `[IBMC]` | ⚠️ **flag** | On ocean/beach-proximate lots, assess risk of shallow water table; geotechnical investigation may be required before steel inspection; confirm requirement with Building Division at permit intake | CBC 2025 § 1803.2 |
| 4a | **FEMA flood zone check** `[IBMC]` | ⚠️ **flag** | If parcel is in FEMA Special Flood Hazard Area (AE/VE zone): require Flood Elevation Certificate. Pool equipment and structural design must account for flood loads per ASCE 7 and CBC Chapter 16. High water table advisory: verify seasonal groundwater depth in geotech report for dewatering and buoyancy considerations. Imperial Beach has significant AE flood zones especially near the coast and Tijuana River. | ASCE 7-22; CBC Ch. 16; 44 CFR 60.3 |
| 5 | **CSS portal submittal** `[IBMC]` | ⚠️ **flag** | All permit applications submitted via CSS portal at https://www.imperialbeachca.gov/225/Building-Division; plans, calculations, and forms uploaded digitally | Imperial Beach CITY.md |
| 6 | **Building permit + electrical permit** `[IBMC]` | ⚠️ **flag** | Building permit required for pool shell/structure; electrical permit required for pool equipment (pump, heater, lighting, bonding); confirm combined vs. separate with Building Division | CEC 2025 Art. 680 |
| 7 | **2025 code edition referenced on plans** `[IBMC]` | ⚠️ **flag** | All submitted plans must reference 2025 California Building Standards Code | Imperial Beach CITY.md |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/imperial-beach/swimming-pool/`.
