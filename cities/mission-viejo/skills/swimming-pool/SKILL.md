---
description: "Swimming Pool permits in Mission Viejo, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Mission Viejo local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Mission Viejo."
version: "1.0"
...
# Mission Viejo — Swimming Pool Permit

Handles swimming pool permits in Mission Viejo end-to-end.
...
- **3** SMWD will-serve / connection — **flag**
- **4** Liquefaction / landslide check — **flag**
- **5** OCFA fuel mod zone — **flag**
- **6** Grading permit — **flag**
- **7** VHFHSZ — **flag**
- **8** OCFA fire review — **flag**
- **10** Drowning prevention measures — **flag**
...
```
## Ready to Submit — Mission Viejo Swimming Pool Permit
...
| 1 | **HOA/ARC approval** `[MVMC]` | ⚠️ **flag** | Most Mission Viejo residential neighborhoods require prior ARC/HOA approval for pool or spa installation; obtain written approval before submitting city permit application | CC&Rs (city does not enforce); CITY.md |
...
| 3 | **SMWD will-serve / connection** `[MVMC]` | ⚠️ **flag** | Pool fill and backwash connections may require SMWD coordination; verify service capacity and connection requirements with Santa Margarita Water District | SMWD; CITY.md |
| 4 | **Liquefaction / landslide check** `[MVMC]` | ⚠️ **flag** | Verify parcel is not in mapped liquefaction or landslide zone; if so, geotechnical investigation required before pool permit | CITY.md — Special Hazards Checklist |
| 5 | **OCFA fuel mod zone** `[MVMC]` | ⚠️ **flag** | If property is adjacent to open space or OCFA fuel modification zone, verify pool placement and associated hardscape do not conflict with fuel mod plan | OCFA; CITY.md |
| 6 | **Grading permit** `[MVMC]` | ⚠️ **flag** | Pool excavation above CBC thresholds requires a separate grading permit under MVMC Ch. 8.10; soils report likely required | MVMC Ch. 8.10; CITY.md |
| 7 | **VHFHSZ** `[MVMC]` | ⚠️ **flag** | Hillside/canyon parcels: pool placement on or adjacent to slopes must comply with CBC slope setback and foundation requirements; OCFA fuel mod compliance required | CAL FIRE; CBC §1808.7; CITY.md |
| 8 | **OCFA fire review** `[MVMC]` | ⚠️ **flag** | OCFA review required for projects in fire-hazard areas; pool equipment placement (gas heater, electrical) must meet clearance requirements | OCFA; CITY.md |
| 10 | **Drowning prevention measures** `[MVMC]` | ⚠️ **flag** | Two prevention features required per Health & Safety Code §115922 (state law); verify valid combinations with Building Services | Health & Safety Code §115922 |

---

## References

Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/mission-viejo/swimming-pool/`.
