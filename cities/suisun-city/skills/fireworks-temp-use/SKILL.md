---
description: "Fireworks Temp Use permits in Suisun City, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Suisun City local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions fireworks temp use in Suisun City."
version: "1.0"
...
# Suisun City — Fireworks Temp Use Permit

Handles fireworks temp use permits in Suisun City end-to-end.
...
```
## Ready to Submit — Suisun City Fireworks Temp Use Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/fireworks-temp-use/`. City overlay source: `skills/cities/suisun-city/fireworks-temp-use/`.
