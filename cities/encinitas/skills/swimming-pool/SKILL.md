---
description: "Swimming Pool permits in Encinitas, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Encinitas local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Encinitas."
version: "1.0"
...
# Encinitas — Swimming Pool Permit

Handles swimming pool permits in Encinitas end-to-end.
...
```
## Ready to Submit — Encinitas Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/encinitas/swimming-pool/`.
