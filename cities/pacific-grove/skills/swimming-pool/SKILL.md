---
description: "Swimming Pool permits in Pacific Grove, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Pacific Grove local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in Pacific Grove."
version: "1.0"
...
# Pacific Grove — Swimming Pool Permit

Handles swimming pool permits in Pacific Grove end-to-end.
...
```
## Ready to Submit — Pacific Grove Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/pacific-grove/swimming-pool/`.
