---
description: "Cleanout permits in Jackson, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Jackson local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions cleanout in Jackson."
version: "1.0"
...
# Jackson — Cleanout Permit

Handles cleanout permits in Jackson end-to-end.
...
```
## Ready to Submit — Jackson Cleanout Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/cleanout/`. City overlay source: `skills/cities/jackson/cleanout/`.
