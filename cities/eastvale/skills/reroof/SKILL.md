---
description: "Reroof permits in Eastvale, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Eastvale local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Eastvale."
version: "1.0"
...
# Eastvale — Reroof Permit

Handles reroof permits in Eastvale end-to-end.
...
- **1** Termite/sheathing inspection — **flag**
- **2** Newer construction — tile weight verification — **flag**
- **3** Permit submittal — **flag**
...
```
## Ready to Submit — Eastvale Reroof Permit
...
| 1 | **Termite/sheathing inspection** `[EMC]` | ⚠️ **flag** | Very Heavy termite zone — on tear-off, inspect sheathing carefully for termite damage. Flag any compromised sheathing for replacement before new underlayment. | 2025 CRC R803; CBC Table 2304.12.1.6 |
| 2 | **Newer construction — tile weight verification** `[EMC]` | ⚠️ **flag** | Post-2000 homes commonly have concrete tile roofs. If switching from tile to composition or vice versa, verify structural framing was designed for the target material weight. | 2025 CRC R301.1; CBC 1607.13 |
| 3 | **Permit submittal** `[EMC]` | ⚠️ **flag** | Eastvale Building Safety: (951) 703-4450. Permit applications via eastvaleca.gov or in-person. Verify current OTC/electronic submittal eligibility for standard reroofs. | City process |
| 4 | **Permit expiration** `[EMC]` | ⚠️ **flag** | Standard California permit expiration applies (180 days to commence, 180 days if suspended/abandoned) unless Eastvale has a local amendment. Confirm at permit counter. | 2025 CRC R105.5 |
| 5 | **Solar panel coordination** `[EMC]` | ⚠️ **flag** | If existing solar PV must be removed for reroof, a separate electrical permit is typically required for panel removal and reinstallation. Confirm with Eastvale Building Safety. | CEC Article 690; city process |
| 6 | **Contractor licensing** `[EMC]` | ⚠️ **flag** | C-39 (Roofing) or B (General Building) license required. Very Heavy termite zone work may require coordination with C-35 (Lathing/Plastering) or licensed pest control if fumigation is triggered by sheathing damage found during tear-off. | B&P Code 7028 |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/eastvale/reroof/`.
