---
description: "Ltia Demolition permits in Rolling Hills, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Rolling Hills local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions ltia demolition in Rolling Hills."
version: "1.0"
...
# Rolling Hills — Ltia Demolition Permit

Handles ltia demolition permits in Rolling Hills end-to-end.
...
```
## Ready to Submit — Rolling Hills Ltia Demolition Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/ltia-demolition/`. City overlay source: `skills/cities/rolling-hills/ltia-demolition/`.
