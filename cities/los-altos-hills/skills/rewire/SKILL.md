---
description: "Rewire permits in Los Altos Hills, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Los Altos Hills local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions rewire in Los Altos Hills."
version: "1.0"
...
# Los Altos Hills — Rewire Permit

Handles rewire permits in Los Altos Hills end-to-end.
...
```
## Ready to Submit — Los Altos Hills Rewire Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/rewire/`. City overlay source: `skills/cities/los-altos-hills/rewire/`.
