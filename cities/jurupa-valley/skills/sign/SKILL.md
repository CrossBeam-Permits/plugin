...
# Jurupa Valley — Sign Permit

Handles sign permits in Jurupa Valley end-to-end.
...
- **1** Commercial zones — sign regulations — **flag**
- **2** Billboard / off-premises signs — **flag**
- **3** Illuminated signs — electrical — **flag**
- **4** Wind load — 110 mph — **flag**
- **5** Specific Plan areas — **flag**
- **6** RCFD — commercial signs — **flag**
...
```
## Ready to Submit — Jurupa Valley Sign Permit
...
| 1 | **Commercial zones — sign regulations** `[JVMC]` | ⚠️ **flag** | Jurupa Valley has multiple commercial zones (C-1/C-P, C-O, C-P-S, C-R, C-T, BP) and industrial zones (I-P, M-M, M-SC). Sign type, size, and placement regulations may vary by zone. Verify zone-specific sign standards in JVMC Title 9 before approving sign design. | JVMC Title 9 (Planning and Zoning) |
| 2 | **Billboard / off-premises signs** `[JVMC]` | ⚠️ **flag** | Billboard and off-premises sign installations are out of scope for standard sign permit track — these require specialty review and are handled separately. Do not process billboard applications through standard sign permit flow. | JVMC sign regulations |
| 3 | **Illuminated signs — electrical** `[JVMC]` | ⚠️ **flag** | Illuminated sign installations require electrical permit in addition to sign permit. Verify licensed C-45 (Sign) or C-10 (Electrical) contractor for sign wiring. LED signs must meet CEC requirements for branch circuit wiring. | CEC 600; CSLB C-45 |
| 4 | **Wind load — 110 mph** `[JVMC]` | ⚠️ **flag** | Sign structures must be designed for Jurupa Valley wind speed of 110 mph (Vult). Monument signs, pole signs, and freestanding signs require structural engineering to demonstrate wind resistance. | CBC Chapter 26 (signs); ASCE 7-22 |
| 5 | **Specific Plan areas** `[JVMC]` | ⚠️ **flag** | Parcels within a Specific Plan area (SP zone) may have supplemental sign standards beyond base JVMC zoning code. Verify SP area design guidelines if applicable. | JVMC Title 9 (SP zones) |
| 6 | **RCFD — commercial signs** `[JVMC]` | ⚠️ **flag** | RCFD fire marshal review is not typically required for standard sign permits. However, signs affixed to or near fire suppression systems, exiting paths, or emergency access routes may require coordination with RCFD. | RCFD; CITY.md |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/sign/`. City overlay source: `skills/cities/jurupa-valley/sign/`.
