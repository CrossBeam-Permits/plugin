---
description: "Reroof permits in Jurupa Valley, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Jurupa Valley local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Jurupa Valley."
version: "1.0"
...
# Jurupa Valley — Reroof Permit

Handles reroof permits in Jurupa Valley end-to-end.
...
- **3** Wind uplift — 110 mph — **flag**
- **4** RCFD fire review — FHSZ — **flag**
- **5** Underlayment — WUI parcels — **flag**
...
```
## Ready to Submit — Jurupa Valley Reroof Permit
...
| 1 | **FHSZ parcel verification** `[JVMC]` | ⚠️ **flag** | Before determining roofing class requirement, verify FHSZ designation at RCFD fire hazard map (rvcfire.org). Jurupa Valley's 2025 OSFM LRA FHSZ maps are being adopted via ordinance introduced May 15, 2025. Parcels in hillside/wildland interface areas of Jurupa Valley are most likely designated. | rvcfire.org; JVMC FHSZ ordinance |
| 3 | **Wind uplift — 110 mph** `[JVMC]` | ⚠️ **flag** | Jurupa Valley design wind speed 110 mph (Vult, Exposure C). Roofing manufacturer's wind uplift certification must be appropriate for this wind zone. Common shingle manufacturers must certify product for ≥ 110 mph at the installed fastening pattern. | CRC Table R301.2(2); CBC wind design |
| 4 | **RCFD fire review — FHSZ** `[JVMC]` | ⚠️ **flag** | For parcels within a designated FHSZ, RCFD may require fire plan check confirmation for re-roofing scope affecting wildfire-rated assemblies. Contact RCFD: RRUOFMPermits@fire.ca.gov or (951) 955-4777. | RCFD; CITY.md |
| 5 | **Underlayment — WUI parcels** `[JVMC]` | ⚠️ **flag** | FHSZ parcels: underlayment must be an approved ember-resistant / fire-rated underlayment in addition to standard CBC requirements. Standard felt underlayment may not be sufficient. | CBC Chapter 7A; manufacturer specs |
| 6 | **Permit portal** `[JVMC]` | ⚠️ **flag** | Submit through Accela Citizen Access: https://aca-prod.accela.com/jurupa/. Re-Roof Residential listed as online self-issue. Contact: (951) 332-6464 or BuildingPermits@jurupavalley.org. | City of Jurupa Valley Building Department |
| 7 | **Contractor licensing** `[JVMC]` | ⚠️ **flag** | Licensed C-39 (Roofing) contractor required. Current CSLB license + Jurupa Valley city business license. | CSLB; H&S Code 1900 |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/jurupa-valley/reroof/`.
