---
description: "Reroof permits in Seal Beach, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Seal Beach local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions reroof in Seal Beach."
version: "1.0"
...
# Seal Beach — Reroof Permit

Handles reroof permits in Seal Beach end-to-end.
...
```
## Ready to Submit — Seal Beach Reroof Permit
...
| 1 | **Leisure World pre-approval** `[SBMC]` | ⚠️ **flag** | If property is in Leisure World (Seal Beach), obtain Architectural Committee pre-approval BEFORE submitting city permit application | Leisure World CC&Rs / Architectural Committee |
| 2 | **Solar panel coordination** `[SBMC]` | ⚠️ **flag** | If existing or new solar panels must be removed/reinstalled for re-roof, a separate solar/electrical permit is required; SolarAPP+ applies for eligible systems | City process / SolarAPP+ |
| 3 | **Coastal zone — CDP check** `[SBMC]` | ⚠️ **flag** | Confirm property is not in a CDP-required scenario. Routine like-for-like reroof is exempt; material/type change may trigger CCC review | Coastal Act §30610 |
...
| 6 | **OCFA** `[SBMC]` | ⚠️ **flag** | Fire authority is OCFA — coordinate any OCFA-specific requirements for WUI or fire zone properties | OCFA |
| 7 | **Open violations check** `[SBMC]` | ⚠️ **flag** | If unpermitted work is discovered during plan review or field inspection (e.g., unpermitted dormer, addition, or structural alteration), the reroof permit **cannot be finaled** until the violation is resolved, legalized, or abated. This applies regardless of whether the reroof scope itself is code-compliant. | SBMC / City building department policy |
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/reroof/`. City overlay source: `skills/cities/seal-beach/reroof/`.
