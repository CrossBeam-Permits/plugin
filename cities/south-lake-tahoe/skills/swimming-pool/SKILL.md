---
description: "Swimming Pool permits in South Lake Tahoe, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + South Lake Tahoe local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions swimming pool in South Lake Tahoe."
version: "1.0"
...
# South Lake Tahoe — Swimming Pool Permit

Handles swimming pool permits in South Lake Tahoe end-to-end.
...
```
## Ready to Submit — South Lake Tahoe Swimming Pool Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/swimming-pool/`. City overlay source: `skills/cities/south-lake-tahoe/swimming-pool/`.
