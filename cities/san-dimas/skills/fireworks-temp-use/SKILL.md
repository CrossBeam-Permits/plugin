---
description: "Fireworks Temp Use permits in San Dimas, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + San Dimas local amendments, produces a portal-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions fireworks temp use in San Dimas."
version: "1.0"
...
# San Dimas — Fireworks Temp Use Permit

Handles fireworks temp use permits in San Dimas end-to-end.
...
```
## Ready to Submit — San Dimas Fireworks Temp Use Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/fireworks-temp-use/`. City overlay source: `skills/cities/san-dimas/fireworks-temp-use/`.
