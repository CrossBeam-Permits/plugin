---
description: "Patio Cover permits in Bell, CA. Works in two modes: (A) BUILDER/HOMEOWNER — walks through project scoping, checks compliance against state code + Bell local amendments, produces an Accela-ready submission guide; (B) CITY REVIEWER — runs the plan-check checklist against a submittal and produces tight corrections output. Inspection items deferred (not listed as corrections). Use when the user mentions patio cover in Bell."
version: "1.0"
...
# Bell — Patio Cover Permit

Handles patio cover permits in Bell end-to-end.
...
```
## Ready to Submit — Bell Patio Cover Permit
...
Deep-dive references are in `references/` — load on demand for specific code questions. State skill source: `skills/california/patio-cover/`. City overlay source: `skills/cities/bell/patio-cover/`.
